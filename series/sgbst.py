...
browse_history_count = browse_history.loc[:, ['userid', 'browse_data', 'browse_tag']].groupby(['userid']).sum()
...
bill_detail_mean = bill_detail.groupby(['userid']).max()
...
loan_data = user_info.join(bill_detail_mean, how = 'outer')
